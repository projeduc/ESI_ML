...
\begin{frame}
	...
	\begin{figure}
		\centering
		\hgraphpage[.75\textwidth]{CRISP-DM-tasks_}
...
\begin{frame}
	...
	\begin{itemize}
		\item \optword{Completeness}: is all required data present?
		\begin{itemize}
			\item Sufficient number of samples.
			\item Less percentage of missing values.
		\end{itemize}
		\item \optword{Validity}: does the data match the required format for the business?
		\begin{itemize}
			\item Number and type of features (nominal, binary, ordinal or continuous).
		\end{itemize}
		\item \optword{Accuracy}: correctness of the data values based on the agreed upon "source of truth".
		\item \optword{Uniqueness}: are all features unique?
		\item \optword{Consistency}: is the data consistent?
		\begin{itemize}
			\item Data from many sources must have the same format (scale, etc.).
		\end{itemize}
		\item \optword{Timeliness}: is the data up to date?
	\end{itemize}
